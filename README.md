...
- Optional temporary essay archive import through a separately installed Codex CLI: allowlisted administrators preview a bounded archive sample, request a scan, review the proposed book/author associations, then explicitly copy selected essays into the club forum. With webhooks enabled, headers, essay text and files use the author's nickname and avatar. Generated source links are omitted from posts; originals and internal source bindings remain. The importer is disabled by default and has a persistent launch quota; ordinary chat messages never invoke Codex. Failed scans can accept an audited human plan, and completed imports can be restyled in their existing topics, without another model call.
